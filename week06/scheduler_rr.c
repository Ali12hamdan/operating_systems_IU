...
ProcessData find_next_process_rr() {
    int location = -1;
    for (int i = 0; i < data_size; i++) {
        if (data[i].burst > 0 && data[i].at <= total_time) {
            if (data[i].remaining_quantum > 0 || data[i].remaining_quantum == -1) {
                location = i;
                break;
            }
        }
    ...
    if (location == -1) {
        return data[running_process]; // No process found
    }
...
void schedule_handler_rr(int signum)
{
    total_time++;
    ...
    if (running_process >= 0) {
        ...
        if (data[running_process].burst == 0) {
            ...
            data[running_process].remaining_quantum = quantum; // Reset the quantum slice for the process
        }
    }

    check_burst();
    ProcessData next_process = find_next_process_rr();
...
int main(int argc, char *argv[]) {
    ...
    printf("Enter the quantum for the Round-Robin scheduler: ");
    scanf("%u", &quantum);
    ...
    signal(SIGALRM, schedule_handler_rr);
